...
void setup_async_server() {
    ...
    if (obj["fire"]) {
      ...
      // initial 2 bytes of all 1's for start detection:
      Serial.write(255);
      Serial.write(255);
      // sign byte:
      if (total_step_count > 0) Serial.write((byte)0x00);
      else Serial.write(1);

      if (abs(total_step_count) < 256) Serial.write((byte)0x00);
      else Serial.write(abs(total_step_count) / 256);  // MSB

      if (abs(total_step_count) % 256 == 0) Serial.write((byte)0x00);
      else Serial.write(abs(total_step_count) % 256);   //LSB
      ...
      delay(200); // so that esp doesnt keep sending too many command to
